...
func add_item(player_name, score):
	...
	if list_index == 1:
		item.self_modulate = Color(0.93, 0.88, 0.09)
	elif list_index == 2:
		item.self_modulate = Color(0.43, 0.43, 0.43)
	elif list_index == 3:
		item.self_modulate = Color(0.37, 0.27, 0.06)
	else:
		item.self_modulate = Color(0.21, 0.84, 0.23)
	$"Board/HighScores/ScoreItemContainer".add_child(item)


func add_no_scores_message():
	var item = $"Board/MessageContainer/TextMessage"
	item.text = "Ainda ninguém se registou aqui! Sê o primeiro a entrar neste painel."
